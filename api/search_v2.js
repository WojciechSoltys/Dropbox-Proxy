...
export default async function handler(req, res) {
  try {
    ...
    if (!query || !options?.path) return res.status(400).json({ error: "Missing 'query' or 'options.path'" });
    if (!isAllowed(options.path)) return res.status(403).json({ error: "Path not allowed", allowed_prefixes: ALLOWED });
    ...
    const r = await fetch("https://api.dropboxapi.com/2/files/search_v2", {
      ...
      body: JSON.stringify({ query, options: { ...options, max_results: options?.max_results || 100 } })
    });
